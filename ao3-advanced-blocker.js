...
(function () {
  ...
  function checkPrimaryPairing(categorizedTags, config) {
    ...
    for (const entry of primaryCharacters) {
      const { key, condObj } = getConditionInfo(entry);
      const item = entry.character || entry.block;
      if (item) {
    ...
    // If all relationship requirements have fandom conditions and none match
    // the current work's fandom, skip pairing blocking entirely
    if (
      relGroups.size > 0 &&
      !hasGlobalRel &&
      !Array.from(relGroups.entries()).some(
        ([key, group]) =>
          key !== "global" && conditionMatchesFandom(group.condObj, fandomTags),
      )
    ) {
      return null;
    }
    ...
    if (
      charGroups.size > 0 &&
      !hasGlobalChar &&
      !Array.from(charGroups.entries()).some(
        ([key, group]) =>
          key !== "global" && conditionMatchesFandom(group.condObj, fandomTags),
      )
    ) {
      ...
      if (conditionMatchesFandom(group.condObj, fandomTags)) {
        if (
          group.items.some((item) =>
            relationshipTags.some((t) => matchPattern(t, item, true)),
          )
        ) {
